...
**In activity 4 we used read.csv instead of read\_csv function**:
...
``` r
college_recent_grads %>% 
  summarise(mean_med_earnings = mean(median), std_med_earnings = sd(median), min_med_earnings = min(median), median_median_earnings = median(median), max_med_earnings = max(median))
```

    ## # A tibble: 1 x 5
    ##   mean_med_earnings std_med_earnings min_med_earnings median_median_earnings
    ##               <dbl>            <dbl>            <dbl>                  <dbl>
...
**Its gonna be mostly close to median but will have values on higher end
as well**:
...
college_recent_grads %>% 
  ...
  summarise(mean_med_earnings = mean(median), std_med_earnings = sd(median), min_med_earnings = min(median), median_median_earnings = median(median), max_med_earnings = max(median)) %>% 
  arrange(median_median_earnings) %>% 
  knitr::kable()
```

| major\_category                     | mean\_med\_earnings | std\_med\_earnings | min\_med\_earnings | median\_median\_earnings | max\_med\_earnings |
|:------------------------------------|--------------------:|-------------------:|-------------------:|-------------------------:|-------------------:|
...
**Highest median earning is for major “Law & Public Policy and the least
median income is for major”Psychology & Social Work"**:
...
**It displays the output table as html table.**:
...
college_recent_grads %>% 
  ...
  summarise(mean_med_earnings = mean(median), std_med_earnings = sd(median), min_med_earnings = min(median), median_median_earnings = median(median), max_med_earnings = max(median)) %>% 
  arrange(median_median_earnings) %>% 
  ggplot(aes(x=major_category, y=median_median_earnings, fill(major_category))) + 
  geom_boxplot() + 
  geom_jitter()
```
